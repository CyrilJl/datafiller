...
Fitting one model per missingness pattern could be expensive, so the default numeric path avoids refitting from scratch. Ridge regression depends on the training data only through its Gram matrix, so `datafiller` accumulates the Gram matrix of the augmented matrix ``[X, y, 1]`` once per column over the rows that are complete on **all** candidate features, then adds the rows each pattern additionally admits and solves the ridge system directly. Custom regressors and categorical targets are fitted on the materialized training subset instead, since arbitrary estimators cannot be trained from summary statistics.

Those per-pattern additions overlap heavily — a row whose missing values sit in columns *N* is a valid training row for every pattern that excludes a superset of *N* — so the training rows are grouped by their exact missingness pattern and each group's contribution to the Gram matrix is accumulated **once** and reused by every pattern that admits it. Groups with a single row, and groups beyond a memory budget, are accumulated on demand instead. Candidate rows and groups are found from an index of missing positions keyed by their first missing column, so the work per pattern scales with the number of missing values in that pattern's excluded columns rather than with the number of training rows.

The Gram matrices are accumulated and solved in double precision: a pattern's Gram matrix is a sum of many contributions, and the intercept correction cancels most of their magnitude, which single precision does not survive on data that is not centred.

Full-matrix work is kept to a minimum of passes. One traversal of the input yields the missing-value mask, the per-column counts and sums that standardization and feature scoring need, and the check for infinities. Standardization writes the standardized matrix and the original-scale output buffer from a single read, and imputed values are rescaled as they are written, so no denormalization pass over the whole matrix is needed. Feature-selection scores are accumulated from masked moments, never materializing a full-size temporary. Performance-critical index manipulations and these passes are accelerated with Numba throughout, in parallel where the work allows.
